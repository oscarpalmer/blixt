...
var Blixt = (() => {
  ...
  function getKey(...parts) {
    return parts.filter((part) => part !== void 0).map((part) => getString(part).trim()).filter((part) => part.length > 0).join(period);
  }
  ...
  function handleArray(parameters) {
    ...
    switch (callback) {
      ...
      case "splice": {
        return (start, remove, ...items) => synthetic(
          start,
          remove,
  ...
  function transformItem(state, prefix, key, value) {
    ...
    return typeof value === "object" ? createStore(value, state, getKey(prefix, key)) : value;
  }
  ...
  function getNodes(value) {
    ...
    return array.map(
      (item) => item instanceof DocumentFragment ? Array.from(item.childNodes) : [item]
    );
  }
  function mapAttributes(element, expressions) {
    ...
    for (const attribute of attributes2) {
      ...
      if (!(expression instanceof Expression) || !(element instanceof HTMLElement)) {
        continue;
      }
  ...
  function observeContent(comment2, expression) {
    ...
    observe(expression.value, (value) => {
      ...
      if (value === void 0 || value === null || isArray) {
        isText = false;
        current = isArray && value.length > 0 ? updateArray(comment2, current, value) : current === void 0 ? void 0 : replaceNodes(current, [[comment2]], false);
        return;
      }
